...
def download_file(url, base_filename):
    ...
    try:
        ...
        if extension_from_content in ['.xls', '.xlsx']:
            ...
            final_extension = '.xlsx'  # Default to .xlsx for employment data
...
def main():
    ...
    # Path to the Excel file in the Data folder (adjust filename as needed)
    excel_file = os.path.join(project_dir, "Data", "Employment_sources.xlsx")
    ...
    # Create "Employment" folder inside the existing Data folder
    data_dir = os.path.join(project_dir, "Data")
    employment_dir = os.path.join(data_dir, "Employment")
    
    if not os.path.exists(employment_dir):
        os.makedirs(employment_dir)
        print(f"Created Employment folder: {employment_dir}")
    else:
        print(f"Employment folder already exists: {employment_dir}")
    
    # Change to Employment directory
    os.chdir(employment_dir)
    print(f"Changed to Employment directory: {os.getcwd()}")
    ...
    for index, row in df.iterrows():
        ...
        print(f"\n{'='*60}")
        print(f"Processing row {index + 1}/{len(df)}: {oblast}")
        print(f"{'='*60}")
        
        # Generate base filename (without extension) - Oblast name only
        base_filename = oblast
        ...
        if success:
            ...
            print(f"✗ Failed to download file for {oblast}")
            failed_downloads += 1
            failed_downloads_list.append(oblast)  # Add to failures list
    ...
    report_filename = f"Employment_download_report_{timestamp}.txt"
    
    # Write comprehensive report to file
    with open(report_filename, 'w', encoding='utf-8') as report_file:
        report_file.write("="*60 + "\n")
        report_file.write("EMPLOYMENT DATA DOWNLOAD REPORT\n")
        report_file.write("="*60 + "\n")
        report_file.write(f"Report generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        report_file.write(f"Employment directory: {employment_dir}\n")
        ...
        if failed_downloads > 0:
            ...
            report_file.write("Failed oblasts:\n")
    ...
    print(f"\n{'='*60}")
    print("EMPLOYMENT DATA DOWNLOAD PROCESS COMPLETED!")
    print(f"{'='*60}")
    print(f"✓ Report saved to: {report_filename}")
    print(f"✓ Employment directory: {employment_dir}")
