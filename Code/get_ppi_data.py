...
def download_file(url, base_filename):
    ...
    try:
        ...
        if extension_from_content in ['.xls', '.xlsx']:
            ...
            final_extension = '.xlsx'  # Default to .xlsx for PPI data
...
def main():
    ...
    excel_file = os.path.join(project_dir, "Data", "PPI.xlsx")
    ...
    # Create "PPI" folder inside the existing Data folder
    data_dir = os.path.join(project_dir, "Data")
    ppi_dir = os.path.join(data_dir, "PPI")
    
    if not os.path.exists(ppi_dir):
        os.makedirs(ppi_dir)
        print(f"Created PPI folder: {ppi_dir}")
    else:
        print(f"PPI folder already exists: {ppi_dir}")
    
    # Change to PPI directory
    os.chdir(ppi_dir)
    print(f"Changed to PPI directory: {os.getcwd()}")
    ...
    report_filename = f"PPI_download_report_{timestamp}.txt"
    
    # Write comprehensive report to file
    with open(report_filename, 'w', encoding='utf-8') as report_file:
        report_file.write("="*60 + "\n")
        report_file.write("PPI DATA DOWNLOAD REPORT\n")
        report_file.write("="*60 + "\n")
        report_file.write(f"Report generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        report_file.write(f"PPI directory: {ppi_dir}\n")
    ...
    print(f"\n{'='*60}")
    print("PPI DATA DOWNLOAD PROCESS COMPLETED!")
    print(f"{'='*60}")
